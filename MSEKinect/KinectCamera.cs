...
namespace MSEKinect
{
    public class KinectCamera
    {
        ...
        List<Person> currentTrackedPersons = new List<Person>();
        ...
        List<Person> ProcessPersonsOnFrame(List<Person> updatedPersons, List<Person> currentPersons)
        {
           ...
           //For each Person in the list, remove it's held-device referece and the circular reference held by the device
            foreach (Person p in missingPersons)
            {
                ...
                if (d != null)
                {
                    ...
                    //TODO Handle the required disconnect message to the device
                    //d.SendDisconnectMessage(); 
                }
        ...
        void ks_SkeletonFrameReady(object sender, SkeletonFrameReadyEventArgs e)
        {
            ...
            else
            {
                ...
                currentTrackedPersons = ProcessPersonsOnFrame(updatedPersons, currentTrackedPersons); 
            }
        }
